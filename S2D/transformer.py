...
from torch.overrides import (
    has_torch_function, has_torch_function_variadic, has_torch_function_unary,
...
def _scaled_dot_product_attention(
        ...
        dropout_p: float = 0.0,
        dis_attn_mask: Optional[Tensor] = None,
) -> Tuple[Tensor, Tensor]:
    r"""
    ...
    if attn_mask is not None:
        attn += attn_mask            #attn_mask:M_ij
    attn = softmax(attn, dim=-1)
    dis_attn_mask[dis_attn_mask < 1.0] = 1.0
    dis_attn_mask = 1.0 / dis_attn_mask    #1/D_ij
    z = attn * dis_attn_mask
    z = z.sum(-1).unsqueeze(2)
    attn = (attn * dis_attn_mask) / z   #（6）的A
    if dropout_p > 0.0:
...
def multi_head_attention_forward(
    ...
    # set up shape vars
    dis_attn_mask = attn_mask.clone()
    delta=3
    attn_mask[dis_attn_mask <= delta] = 0.0
    attn_mask[dis_attn_mask > delta] = -100000000.0
    ...
    #
    attn_output, attn_output_weights = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, dis_attn_mask)#transformer的第l层输出
