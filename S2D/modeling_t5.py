...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if past_prompt is not None:
            ...
            if syntax_ids is not None and self.embed_syntax is not None:
                prefix_embeds = self.embed_syntax(syntax_ids)
                batch_size, seq_length = input_shape
                batch_size, prefix_length = syntax_ids.size()
                seq_length = seq_length + prefix_length
            ...
            # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
            # ourselves in which case we just need to make it broadcastable to all heads.
            extended_attention_mask = self.get_extended_attention_mask(attention_mask, input_shape,device=inputs_embeds.device)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared)#和1692行综合起来，这两行代码初始化了一个T5模型的编码器部分，其中包括一个嵌入层用于处理输入标记，并一个由多个Transformer层组成的堆栈（T5Stack），负责对输入序列进行编码。这是Transformer模型的一般结构，用于处理文本到文本的各种任务。
