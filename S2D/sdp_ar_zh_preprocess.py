...
def process_train(dataset, tokenizer, upos2id, deprel2id=None, language='en', max_token_len=350, max_prefix_len=5):
    ...
    for data in tqdm(dataset):
        ...
        if flag<=1743:#k  ar_SRL
            root = trigger_span[0] + 1
        else:#k  ar_SRL
            root = trigger_span[0][0] + 1#k  ar_SRL
        ...
        if flag<=1743:
            ...
            sdp_prefix_ids = sdp_prefix_ids + [start_ids[trigger_span[0][0]]] * (sdp_max_prefix_len - len(sdp_prefix_ids))#k
        ...
        if flag<=1743:
            ...
            prefix_ids = prefix_ids + [start_ids[trigger_span[0][0]]] * (max_prefix_len - len(prefix_ids))
...
def process_test(dataset, tokenizer, upos2id, deprel2id=None, language='en', max_token_len=350, max_prefix_len=5):
    sdp_max_prefix_len=20#k
    sdp = hanlp.load('SEMEVAL16_ALL_ELECTRA_SMALL_ZH')#k
    ...
    for data in tqdm(dataset):
        ...
        sdp_g=nx.Graph()#k
        for i in range (len(sdp_doc)):#k
            for j in range (len(sdp_doc[i]['deps'])):
                sdp_g.add_edge(i+1, sdp_doc[i]['deps'][j][0])#k
                sdp_g.add_edge(sdp_doc[i]['deps'][j][0], i+1)#k
