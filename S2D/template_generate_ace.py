...
class event_template_generator():
    def __init__(self, passage, triggers, roles, input_style, output_style, vocab, lang, encoder_pos_ids,
        ...
        for event in self.events:
            ...
            self.event_templates.append(
                theclass(self.input_style, self.output_style, event['tokens'], event['event type'], self.lang, event))

        self.data = [x.generate_pair(x.trigger_text) for x in self.event_templates]
        self.data = [x for x in self.data if x]
        for i in range(len(self.data)):
            self.data[i][2]['encoder_pos_ids'] = encoder_pos_ids[0]
            self.data[i][2]['encoder_input_ids'] = encoder_input_ids[0]
            self.data[i][2]['encoder_attention_mask'] = encoder_attention_mask[0]
            self.data[i][2]['prefix_ids'] = prefix_ids[0]
            self.data[i][2]['encoder_syntax_mask'] = encoder_syntax_mask[0]
    ...
    def process_events(self, passage, triggers, roles):
        ...
        for trigger, arguments in events.items():
            eve_type = trigger[2]
            eve_text = ''.join(passage[trigger[0]:trigger[1]]) if self.no_space else ' '.join(
                passage[trigger[0]:trigger[1]])
            ...
            for argument in arguments:
                ...
                argus[role_type].append({
                    'argument text': ''.join(passage[argument[1][0]:argument[1][1]]) if self.no_space else ' '.join(
                        passage[argument[1][0]:argument[1][1]]),
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Place--> [None] </--Place-->')
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Person']]) if "Person" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        )
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Agent']]) if "Agent" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Victim']]) if "Victim" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Instrument']]) if "Instrument" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Agent--> {} </--Agent--> <--Victim--> {} </--Victim--> <--Instrument--> {} </--Instrument--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Agent--> [None] </--Agent--> <--Victim--> [None] </--Victim--> <--Instrument--> [None] </--Instrument--> <--Place--> [None] </--Place-->')  # Person is a mislabeled case.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Artifact--> [None] </--Artifact--> <--Destination--> [None] </--Destination--> <--Origin--> [None] </--Origin--> <--Vehicle--> [None] </--Vehicle--> <--Agent--> [None] </--Agent--> <--Place--> [None] </--Place-->')  # Entity is a mislabeled case.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Artifact']]) if "Artifact" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Destination']]) if "Destination" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Origin']]) if "Origin" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Vehicle']]) if "Vehicle" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Agent']]) if "Agent" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        ...
                        output_texts.append(
                            "<--Artifact--> {} </--Artifact--> <--Destination--> {} </--Destination--> <--Origin--> {} </--Origin--> <--Vehicle--> {} </--Vehicle--> <--Agent--> {} </--Agent--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Buyer--> [None] </--Buyer--> <--Artifact--> [None] </--Artifact--> <--Seller--> [None] </--Seller--> <--Place--> [None] </--Place--> <--Beneficiary--> [None] </--Beneficiary-->')  # Org is mislabeled
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Buyer']]) if "Buyer" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Artifact']]) if "Artifact" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Seller']]) if "Seller" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Beneficiary']]) if "Beneficiary" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Buyer--> {} </--Buyer--> <--Artifact--> {} </--Artifact--> <--Seller--> {} </--Seller--> <--Place--> {} </--Place--> <--Beneficiary--> {} </--Beneficiary-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Giver--> [None] </--Giver--> <--Recipient--> [None] </--Recipient--> <--Place--> [None] </--Place--> <--Beneficiary--> [None] </--Beneficiary-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Giver']]) if "Giver" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Recipient']]) if "Recipient" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Beneficiary']]) if "Beneficiary" in argu.keys() else NO_ROLE
                        ...
                        output_texts.append(
                            "<--Giver--> {} </--Giver--> <--Recipient--> {} </--Recipient--> <--Place--> {} </--Place--> <--Beneficiary--> {} </--Beneficiary-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Agent--> [None] </--Agent--> <--Org--> [None] </--Org--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Agent']]) if "Agent" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Org']]) if "Org" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Agent--> {} </--Agent--> <--Org--> {} </--Org--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Org--> [None] </--Org--> <--Place--> [None] </--Place-->')  # Agent is mislabel.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Attacker--> [None] </--Attacker--> <--Target--> [None] </--Target--> <--Instrument--> [None] </--Instrument--> <--Place--> [None] </--Place-->')  # victim is mislabel.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
                ...
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        filler = (
                            ...
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Target']]) if "Target" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Instrument']]) if "Instrument" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        ...
                        output_texts.append(
                            "<--Attacker--> {} </--Attacker--> <--Target--> {} </--Target--> <--Instrument--> {} </--Instrument--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Entity--> [None] </--Entity--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Entity']]) if "Entity" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE
                        )
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Entity--> [None] </--Entity--> <--Place--> [None] </--Place-->')  # Agent is mislabel
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Person--> {} </--Person--> <--Entity--> {} </--Entity--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Entity--> [None] </--Entity--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Agent--> [None] </--Agent--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Person--> {} </--Person--> <--Agent--> {} </--Agent--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Entity--> [None] </--Entity--> <--Place--> [None] </--Place-->')  # defendant and adjudacator is mislabel
        return ('\n'.join(output_template.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Defendant--> [None] </--Defendant--> <--Prosecutor--> [None] </--Prosecutor--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Defendant']]) if "Defendant" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Prosecutor']]) if "Prosecutor" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Adjudicator']]) if "Adjudicator" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Defendant--> {} </--Defendant--> <--Prosecutor--> {} </--Prosecutor--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Defendant--> [None] </--Defendant--> <--Plaintiff--> [None] </--Plaintiff--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')  # Prosecutor is mislabel.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
                    ...
                    for argu in self.arguments:
                        filler = (
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Defendant']]) if "Defendant" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Plaintiff']]) if "Plaintiff" in argu.keys() else NO_ROLE,
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Adjudicator']]) if "Adjudicator" in argu.keys() else NO_ROLE
                        ...
                        output_texts.append(
                            "<--Defendant--> {} </--Defendant--> <--Plaintiff--> {} </--Plaintiff--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Defendant--> [None] </--Defendant--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Defendant--> {} </--Defendant--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Entity--> [None] </--Entity--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')  # Defendant is mislabel.
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Entity--> {} </--Entity--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Destination--> [None] </--Destination--> <--Origin--> [None] </--Origin--> <--Agent--> [None] </--Agent-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Person--> {} </--Person--> <--Destination--> {} </--Destination--> <--Origin--> {} </--Origin--> <--Agent--> {} </--Agent-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Plaintiff--> [None] </--Plaintiff--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Plaintiff--> {} </--Plaintiff--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
