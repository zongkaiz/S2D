    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Giver--> [None] </--Giver--> <--Recipient--> [None] </--Recipient--> <--Place--> [None] </--Place--> <--Beneficiary--> [None] </--Beneficiary--> <--Thing--> [None] </--Thing-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
                ...
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        filler = (
                            ...
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Thing']]) if "Thing" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Giver--> {} </--Giver--> <--Recipient--> {} </--Recipient--> <--Place--> {} </--Place--> <--Beneficiary--> {} </--Beneficiary--> <--Thing--> {} </--Thing-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Attacker--> [None] </--Attacker--> <--Target--> [None] </--Target--> <--Instrument--> [None] </--Instrument--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Entity--> [None] </--Entity--> <--Place--> [None] </--Place--> <--Audience--> [None] </--Audience-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
                    ...
                    for argu in self.arguments:
                        filler = (
                            ...
                            f" {AND} ".join(
                                [a['argument text'] for a in argu['Place']]) if "Place" in argu.keys() else NO_ROLE,
                            f" {AND} ".join([a['argument text'] for a in
                                             argu['Audience']]) if "Audience" in argu.keys() else NO_ROLE
                        )
                        output_texts.append(
                            "<--Entity--> {} </--Entity--> <--Place--> {} </--Place--> <--Audience--> {} </--Audience-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Entity--> [None] </--Entity--> <--Place--> [None] </--Place--> <--Adjudicator--> [None] </--Adjudicator-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Defendant--> [None] </--Defendant--> <--Prosecutor--> [None] </--Prosecutor--> <--Place--> {} </--Place--> <--Adjudicator--> {} </--Adjudicator-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Artifact--> [None] </--Artifact--> <--Agent--> [None] </--Agent--> <--Place--> [None] </--Place-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Artifact--> {} </--Artifact--> <--Agent--> {} </--Agent--> <--Place--> {} </--Place-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Artifact--> [None] </--Artifact--> <--Destination--> [None] </--Destination--> <--Origin--> [None] </--Origin--> <--Agent--> [None] </--Agent--> <--Instrument--> [None] </--Instrument-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Artifact--> {} </--Artifact--> <--Destination--> {} </--Destination--> <--Origin--> {} </--Origin--> <--Agent--> {} </--Agent--> <--Instrument--> {} </--Instrument-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
    ...
    def get_output_template(self):
        ...
        for o_style in OUTPUT_STYLE_SET:
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    output_template += ' \n {}'.format(
                        '<--Person--> [None] </--Person--> <--Destination--> [None] </--Destination--> <--Origin--> [None] </--Origin--> <--Agent--> [None] </--Agent--> <--Instrument--> [None] </--Instrument-->')
        return ('\n'.join(output_template.split('\n')[1:])).strip()
            ...
            if o_style in self.output_style:
                if o_style == 'argument:roletype':
                    ...
                    for argu in self.arguments:
                        ...
                        output_texts.append(
                            "<--Person--> {} </--Person--> <--Destination--> {} </--Destination--> <--Origin--> {} </--Origin--> <--Agent--> {} </--Agent--> <--Instrument--> {} </--Instrument-->".format(
                                *filler))
                        gold_sample = True
                    output_str += ' \n {}'.format(' <sep> '.join(output_texts))
        output_str = ('\n'.join(output_str.split('\n')[1:])).strip()
